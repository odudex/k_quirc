 ...
 * flattens a corner into a short diagonal whose points can tie; the corner is
 * their centre rather than whichever came first. */
struct corner_score {
  int best;
  int ties;
  ...
  for (int y = reg->y0; y <= reg->y1; y++) {
    ...
    for (int i = 0; i < 2; i++) {
      if (far) {
        int dx = xs[i] - ref->x;
        int dy = y - ref->y;
        corner_offer(&corners[0], dx * dx + dy * dy, xs[i], y);
      } else {
        int up = xs[i] * ref->x + y * ref->y;
        int rt = xs[i] * -ref->y + y * ref->x;
        corner_offer(&corners[0], up, xs[i], y);
        corner_offer(&corners[1], rt, xs[i], y);
        corner_offer(&corners[2], -up, xs[i], y);
        corner_offer(&corners[3], -rt, xs[i], y);
      }
    }
  }
}
