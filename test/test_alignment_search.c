/* A QR grid whose capstone edges are near-parallel puts the estimated
 * alignment-pattern position - their intersection - far outside the image.
 * The perspective maps then report a "module" of tens of millions of pixels.
 * The alignment search spirals over 100 modules' worth of area: unbounded,
 * that product overflows and the search walks about a billion pixels that do
 * not exist, which on an embedded target is a multi-minute hang.
 *
 * The values below are taken from a watchdog dump on an ESP32-P4: a 600x600
 * frame, the estimate at (8678, 250901), a module of 9842 x 9842 pixels. */
#include <stdio.h>
#include <string.h>
#include <time.h>
...
static void check_search(const char *name, int align_x, int align_y,
  ...
  /* Bounded, the search covers at most the image: well under a millisecond.
   * Unbounded it takes seconds even on a desktop. */
